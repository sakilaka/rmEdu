...
class EbookAudioController extends Controller
{
    public function manageEbookAudio()
    {
        $data['ebooks'] = Ebook::where('user_id', auth()->user()->id)->where('type','ebookaudio')->orderBy('id', 'desc')->get();
        return view('user.ebookaudio.index', $data);
    }
    public function addEbookAudio()
    {
        $data['ebooks'] = Ebook::where('user_id', auth()->user()->id)->where('type','ebookaudio')->orderBy('id', 'desc')->get();
        ...
        return view('user.ebookaudio.create', $data);
    }
    public function storEbookAudio(Request $request)
    {
        $ebook = new Ebook();
        ...
        return redirect()->route('frontend.manage_ebook_audio')->with('message', 'E-Audio created successfully, Thank You.');
    }
    public function editEbookAudio($id)
    {
        $data['ebook'] = $ebook = Ebook::find($id);
        $data['ebooks'] = Ebook::where('user_id', auth()->user()->id)->where('type','ebookaudio')->orderBy('id', 'desc')->get();
        $data["categories"] = Category::where('parent_id', '=' ,0)->where('type','ebook')->get();
        $data["sub_categories"] = Category::where('parent_id',$ebook->category_id)->orderBy('id', 'desc')->get();
        return view('user.ebookaudio.update', $data);
    }
    public function updateEbookAudio(Request $request, $id)
    {
        $ebook = Ebook::find($id);
        $ebook->category_id = $request->category_id;
        $ebook->sub_category_id = $request->sub_category_id;
        ...
        return redirect()->route('frontend.manage_ebook_audio')->with('message', 'E-Audio Update successfully, Thank You.');
    }




    public function deleteEbookAudio(Request $request)
    {
        //dd($request->all());
