...
{
    public function manageEbookVideo()
    {
        $data['ebooks'] = Ebook::where('user_id', auth()->user()->id)->where('type','ebookvideo')->orderBy('id', 'desc')->get();
        return view('user.ebookvideo.index', $data);
    }
    public function addEbookVideo()
    {
        $data['ebooks'] = Ebook::where('user_id', auth()->user()->id)->where('type','ebookvideo')->orderBy('id', 'desc')->get();
        ...
        return view('user.ebookvideo.create', $data);
    }
    public function storEbookVideo(Request $request)
    {
        $ebook = new Ebook();
        ...
        return redirect()->route('frontend.manage_ebook_video')->with('message', 'E-Video created successfully, Thank You.');
    }
    public function editEbookVideo($id)
    {
        $data['ebook'] = $ebook = Ebook::find($id);
        $data['ebooks'] = Ebook::where('user_id', auth()->user()->id)->where('type','ebookvideo')->orderBy('id', 'desc')->get();
        $data["categories"] = Category::where('parent_id', '=' ,0)->where('type','ebook')->get();
        $data["sub_categories"] = Category::where('parent_id',$ebook->category_id)->orderBy('id', 'desc')->get();
        return view('user.ebookvideo.update', $data);
    }
    public function updateEbookVideo(Request $request, $id)
    {
        $ebook = Ebook::find($id);
        $ebook->category_id = $request->category_id;
        $ebook->sub_category_id = $request->sub_category_id;
        ...
        return redirect()->route('frontend.manage_ebook_video')->with('message', 'E-Video Update successfully, Thank You.');
    }




    public function deleteEbookVideo(Request $request)
    {
        //dd($request->all());
        ...
        $ebook->delete();
        return back()->with('message','E-book Video Deleted Successfully');
    }
}
