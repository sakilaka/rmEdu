...
class NanyReportController extends Controller
{
    function report(Request $request)
    {
       return view("Backend.reports.nany");
    }

    function getNanyServiceData(Request $request){
        $service =  Service::where('template',"nany-service")->first();
       ...
       // $totalFiltered = ServiceBooking::where('service_id',$service->id)->count();
        if($search){
            $datas = ServiceBooking::where('service_id',$service->id)
                ->leftJoin('nanies','nanies.id','service_bookings.relation_id')
                ->leftJoin('users','users.id','service_bookings.user_id')
                ->where(function($q) use ($search) {
                    $q->where('invoice_code','like','%'.$search.'%')
                    ->orWhere('nanies.fname','like','%'.$search.'%')
                    ->orWhere('nanies.mobile','like','%'.$search.'%')
                    ->orWhere('nanies.email','like','%'.$search.'%')
        ...
        foreach ($datas as $n_data)
        {
            ...
            $nestedData['nany_name'] = $n_data->nany->fname.' '. $n_data->nany->lname;
            $nestedData['nany_mobile'] = $n_data->nany->mobile;
            $nestedData['nany_email'] = $n_data->nany->email;
    ...
    {
        // $service =  Service::where('template',"nany-service")->first();
        // $data['nany_services'] = ServiceBooking::where('service_id',$service->id)->get();
       return view("Backend.reports.nany_item");
    }
    function getNanyPackageData(Request $request){
        $service =  Service::where('template',"nany-service")->first();
        ...
        foreach ($datas as $n_data)
        {
            ...
            $nestedData['nany_name'] = $n_data->servicebooking->nany->fname.' '. $n_data->nany->lname;
