...
class DiagnosticReportController extends Controller
{
    ...
    {
       return view("Backend.reports.diagnostic");
    }
    function getDiagnosticData(Request $request){
        $service =  Service::where('template',"diagnostic")->first();
       ...
       // $totalFiltered = ServiceBooking::where('service_id',$service->id)->count();
        if($search){
            $datas = ServiceBooking::where('service_id',$service->id)
                ->leftJoin('labs','labs.id','service_bookings.relation_id')
                ->leftJoin('users','users.id','service_bookings.user_id')
                ->where(function($q) use ($search) {
                    $q->where('invoice_code','like','%'.$search.'%')
                    ->orWhere('labs.name','like','%'.$search.'%')
                    ->orWhere('labs.mobile','like','%'.$search.'%')
                    ->orWhere('labs.email','like','%'.$search.'%')
        ...
        foreach ($datas as $n_data)
        {
            ...
            $nestedData['lab_name'] = $n_data->lab?->name;
            $nestedData['lab_mobile'] = $n_data->lab?->mobile;
            $nestedData['lab_email'] = $n_data->lab?->email;
    ...
    function getDiagnosticPackageData(Request $request){
        $service =  Service::where('template',"diagnostic")->first();
        ...
        foreach ($datas as $n_data)
        {
            ...
            $nestedData['lab_name'] = $n_data->servicebooking->lab?->name;
