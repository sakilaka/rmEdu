...
class NurseReportControler extends Controller
{
    function report(Request $request)
    {
       return view("Backend.reports.nurse");
    }
     function getNurseData(Request $request){
        $service =  Service::where('template',"nurse")->first();
       ...
       // $totalFiltered = ServiceBooking::where('service_id',$service->id)->count();
        if($search){
            $datas = ServiceBooking::where('service_id',$service->id)
                ->leftJoin('nurses','nurses.id','service_bookings.relation_id')
                ->leftJoin('users','users.id','service_bookings.user_id')
                ->where(function($q) use ($search) {
                    $q->where('invoice_code','like','%'.$search.'%')
                    ->orWhere('nurses.fname','like','%'.$search.'%')
                    ->orWhere('nurses.mobile','like','%'.$search.'%')
                    ->orWhere('nurses.email','like','%'.$search.'%')
        ...
        foreach ($datas as $n_data)
        {
            ...
            $nestedData['nurse_name'] = $n_data->nurse->fname.' '. $n_data->nurse->lname;
            $nestedData['nurse_mobile'] = $n_data->nurse->mobile;
            $nestedData['nurse_email'] = $n_data->nurse->email;
    ...
    function reportPackage(Request $request)
    {
       ...
       return view("Backend.reports.nurse_item");
    }
    function getNursePackageData(Request $request){
        $service =  Service::where('template',"nurse")->first();
        ...
        foreach ($datas as $n_data)
        {
            ...
            $nestedData['nurse_name'] = $n_data->servicebooking->nurse->fname.' '. $n_data->nurse->lname;
