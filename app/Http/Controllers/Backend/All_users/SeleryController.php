...
class SeleryController extends Controller
{
    ...
    {
        $data['selerys'] = User::where('type','5')->orderBy('id', 'desc')->get();
        return view("Backend.all_users.selery.index",$data);
    }
    ...
    public function create()
    {
        ...
        return view("Backend.all_users.selery.create",$data);
    }
    ...
    public function store(Request $request)
    {
      // dd($request->all());
        ...
        try{
            ...
            DB::commit();
            return redirect()->route('admin.selery.index')->with('message','Selery Add Successfully');
        }catch(\Exception $e){
            DB::rollBack();
            dd($e);
    ...
    {
       // dd('hi');
        $data["selery"]= User::find($id);
        return view("Backend.all_users.selery.update",$data);
    }
    ...
    try{
        ...
        DB::commit();
        return redirect()->route('admin.selery.index')->with('message','Selery Update Successfully');
    }catch(\Exception $e){
        DB::rollBack();
       // dd($e);
        return back()->with ('error_message', $e->getMessage());
    ...
    public function destroy(Request $request)
    {
        ...
        $user =  User::find($request->selery_id);
        @unlink(public_path('upload/users/'.$user->image));

        $user->delete();
        return back()->with('message','Selery Profile Deleted Successfully');
    }
