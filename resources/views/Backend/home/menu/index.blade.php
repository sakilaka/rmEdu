...
<head>
    ...
    <title>{{ env('APP_NAME') }} | All Menu</title>
</head>
                ...
                <div class="content-wrapper">
                    <div class="page-header">
                        ...
                        <nav aria-label="breadcrumb">
                            <a href="{{ route('admin.create_menu') }}" class="btn btn-primary btn-fw">
                                ...
                                <tbody>
                                    @foreach ($menus as $item)
                                        <tr role="row" class="odd">
                                            ...
                                            <td>
                                                @if ($item->type == 'header_menu')
                                                    Header Menu
                                                @elseif ($item->type == 'footer_menu')
                                                    Footer Menu
                                                @endif
                                            </td>
                                            <td>{{ $item->name }}</td>
                                            <td class="text-center">{{ $item->position }}</td>
                                            <td class="text-center">{{ $item->column_num }}</td>
                                            <td class="text-center">
                                                @if ($item->status == 1)
                                                    <a href="{{ route('admin.menu_status_toggle', $item->id) }}">
                                            ...
                                            <td class="text-right">
                                                <a href="{{ route('admin.edit_menu', $item->id) }}"
                        ...
                        <div class="modal-content">
                            <div class="modal-body text-center">
                                ...
                                <div class="m-t-20 flex">
                                    <form action="{{ route('admin.delete_menu') }}" method="POST" id="deleteForm">
                                        @csrf
