...
<header class="fixed-top-header">
    <div class="position-absolute top-0 start-0 end-0 text-center py-3 d-none" id="search_input_area">
        ...
        <div class="container py-1">
            <form action="{{ route('home.head_search') }}" class="row justify-content-center align-items-center">
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNavDropdown">
                <ul class="navbar-nav mx-auto">
                    ...
                    <li class="nav-item">
                        <a class="nav-link {{ Route::is('frontend.find_university') || Route::is('frontend.all_universities_list') ? 'active' : '' }}"
                            href="{{ route('frontend.find_university') }}">Choose Country</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ Route::is('frontend.learner') ? 'active' : '' }}"
                            href="{{ route('frontend.learner') }}">Applicants</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ Route::is('frontend.instructor') ? 'active' : '' }}"
                            href="{{ route('frontend.instructor') }}">Partner</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ Route::is('frontend.blog') ? 'active' : '' }}"
                            href="{{ route('frontend.blog') }}">Blog</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ Route::is('frontend.event_list') ? 'active' : '' }}"
                            href="{{ route('frontend.event_list') }}">Event</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link {{ Route::is('frontend.faq') ? 'active' : '' }}"
                    ...
                    <div>
                        {{-- <form action="#">
                            ...
                            <a id="google_translate_element" href="javascript:void(0)"
                                class=" btn fw-bold me-2 text-uppercase text-black p-0 d-flex align-items-center">
                            </a>
                        </form> --}}
                        ...
                        @if (Auth::check())
                            ...
                            <div class="profile-card-container" id="profileCardContainer">
                                <div class="profile-card" id="profileCard">
                                    <div class="user-info">
                                        <img src="{{ auth()->user()->image_show }}" alt="User Image">
                                    ...
                                    <div class="user-actions">
                                        <button class="btn-primary-bg"
                                            onclick="location.href='{{ route('user.profile') }}'">View
                                            Profile</button>
                                        <button class="btn-tertiary-bg"
                                            onclick="location.href='{{ route('user.logout') }}'">Logout</button>
                                    </div>
                                </div>
                            </div>


                            <a onclick="location.href='{{ route('user.profile') }}'" href="#" id="userImage">
                                <img style="height:40px;width:40px;border-radius: 50%;border: 1px solid #000;"
                                    src="{{ auth()->user()->image_show }}" alt="image" />
                            </a>

                            <a onclick="location.href='{{ route('user.logout') }}'" class="d-md-none"
                                style="margin-left: 12px;">
                                <img src="{{ asset('frontend/images/logout_icon.svg') }}" alt="">
                            </a>
