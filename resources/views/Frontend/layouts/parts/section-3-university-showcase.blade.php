<div class="container" style="margin-top: 5rem">
    ...
    <h3 class="ca-card-title d-block text-center mb-4 font-dm-sans-title">{{ $home_content->university_title }}</h3>
            ...
            @foreach ($university_list as $university)
                ...
                <div class="single-course-category">
                    <div class="course-media">
                        <img data-lazyloaded="1" src="{{ $university->image_show }}" decoding="async"
                            alt="Business &amp; Management" data-ll-status="loaded" class="entered litespeed-loaded">
                    </div>
                    <div class="course-content mt-3">
                        <p class="">{{ $course_count }} PROGRAM(S)</p>
                        <a href="{{ route('frontend.university_details', $university->id) }}">
                            <h4 class="course-category-title">
                                {{ Illuminate\Support\Str::limit($university->name, 45, '...') }}
                            </h4>
                        </a>
                    </div>
                    <div class="course-location-container">
                        <p class="course-location" style="margin-bottom: 0">
                            {{-- <span>{{ $university->state->name }},</span> --}}
                            {{-- <span>{{ $university->city->name }}</span> --}}
                            <span>{{ Illuminate\Support\Str::limit($university->address, 30, '...') }}</span>
                        </p>
                        <p class="course-location" style="margin-bottom: 0">
                            <span>{{ $university->continent->name }},</span>
                        ...
                        <div class="course-content">
                            ...
                            <div class="btn-wrap">
                                <a class="course-button"
                                    href="{{ route('frontend.university_details', $university->id) }}"><i
